...
// ExpOS patch: production certificates can legitimately carry dozens of DNS
// names. Keep this fixed-capacity/no-allocation, but retain enough entries to
// reach Google's YouTube names instead of silently considering only the first
// three. This changes storage capacity only; all original chain, signature,
// validity, and hostname checks remain active.
pub const MAX_SAN_DNS_NAMES: usize = 96;

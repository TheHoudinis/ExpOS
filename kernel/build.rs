...
fn main() {
    let source = "trust/global_sign_root_r1.pem";
    ...
    let output =
        PathBuf::from(env::var_os("OUT_DIR").expect("OUT_DIR")).join("global_sign_root_r1.der");
    fs::write(output, der).expect("write HTTPS trust anchor DER");
}

...
| `expfs*` | ATA I/O, cache, journal, revisions | persistent Form graph and FIN index | metadata model plus a separate primary-master ATA PIO account/preference state journal landed; general persistent Form storage queued |
...
4. Extend the landed ATA PIO dual-slot account/preference journal into a
   general ExpFS block format with a persistent Form graph, durable
...
8. Create Root/system/interface Forms. Use the Abstracted Silicon Layer (ASL) role described in

...
ExpOS v8.0.0-alpha.10 uses two runnable environments during migration. `make
...
| Packages | `Ayo` boot Package Form and Go `ayo v2` with a 20-package built-in Prism catalog including MouseKit and SessionManager, interactive number/name/search TUI, offline/HTTPS catalogs, checksums/signatures, atomic dependency plans, all 11 commands, version constraints, dependency protection, reconciliation, capability merges, manifests and recovery |
| Graphics | `HexaDisplay` Service Form; 800x600 XRGB8888 Bochs/QEMU framebuffer; original Windows-inspired Prism capability compositor with centered taskbar and launcher, four workspaces, master-stack tiling, floating/fullscreen modes, window dragging, overview, Super-key bindings and ten surfaces; Form-owned Browser, Terminal, Forms, Packages, Settings, System and Games windows; atomic geometry/commit, configure/frame/focus/key/pointer events, z-order and hit testing; VGA text-mode restoration on exit; `desktop`, `displayinfo` |
...
| Desktop apps | Graphical Terminal with Up/Down command history; Form registry; Ayo package catalog view; session Settings; live System Scope; Prism Arcade with native ticking Snake and Pong; clickable Start launcher and taskbar; mouse window controls and game cards |
...
| System | Keyboard-and-mouse graphical login with Operator, Power and Guest capability authority; `users`, `login`, `logout`, `status`, `kstat`, `ps`, `dmesg/bootlog`, `ifconfig`, `netstat`, `mode`, bounded Up/Down shell history, `history`, `uptime`, `env`, `whoami`, `reboot`, `shutdown` |
...
The three alpha accounts are fixed in the kernel and sessions are not durable;
password hashing and persistent account Forms are not yet claimed. Mouse wheel
input and GPU acceleration are also not implemented. UEFI-native boot,

...
| Desktop apps | Dark graphical Terminal, Browser, Form registry, Ayo package catalog, System, Games and Notes; large clickable Settings control center with System, Appearance, Network/Wi-Fi, Bluetooth, Display, Input, Privacy and About pages; real taskbar/status/border/contrast/background/accent/pointer controls retained for the current boot; Up/Down terminal history; every app can close and reopen |
...
| Hardware | `date/clock`, `cpuinfo`, `features/kernelcaps`, `lspci`, `mem/free`, VGA and COM1 consoles, PS/2 keyboard/mouse, RTL8139 bus-master DMA, Ethernet/ARP/static IPv4/ICMP/UDP/DNS/TCP/HTTP, PCI Wi-Fi/Bluetooth class discovery with honest driver/connection state, ANSI serial input and CPUID/control-register reporting |
...
physical Wi-Fi drivers, USB host/Bluetooth data transport, concurrent sockets,
the Go execution-context loader, and a standards-complete web engine remain

...
| Graphics | `HexaDisplay` Service Form; 1024x768 Bochs/QEMU framebuffer with XRGB8888, ARGB8888 and RGB565 protocol formats; original black-by-default Prism shell with Start launcher and taskbar, no default or pinned apps, movable/minimizable/maximizable/closeable windows, Notes and eleven surfaces; atomic geometry/commit, configure/frame/focus/key/pointer events, z-order and hit testing; VGA text-mode restoration on exit; `desktop`, `displayinfo` |
...
| Desktop apps | Black-by-default graphical Terminal, Browser, Form registry, Ayo package catalog, Settings, System Scope, Prism Arcade and Notes; Up/Down terminal history; clickable launcher/taskbar; every app can close and reopen |
...
| Hardware | `date/clock`, `cpuinfo`, `features/kernelcaps`, `lspci`, `mem/free`, VGA and COM1 consoles, PS/2 keyboard/mouse, RTL8139 bus-master DMA, Ethernet/ARP/static IPv4/ICMP, ANSI serial input and CPUID/control-register reporting |
| System | Graphical-or-console boot chooser and mutually switchable login, Operator/Power/Guest capability authority, runtime accounts, `users`, `useradd`, `userdel`, `passwd`, `login`, `logout`, `status`, `kstat`, `ps`, `dmesg/bootlog`, `ifconfig`, `ping`, `netstat`, history, `whoami`, `reboot`, `shutdown` |
...
TCP/UDP, DNS, DHCP, TLS, physical Wi-Fi, the Go execution-context loader, and a
